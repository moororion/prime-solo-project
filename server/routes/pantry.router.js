...
// GET route to retrieve pantry items
router.get('/', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM pantry WHERE user_id = $1 ORDER BY expiration_date ASC', [req.user.id]);
    res.json(result.rows);
  } catch (error) {
    console.error('Error retrieving pantry items:', error);
    res.status(500).send('Server error retrieving pantry items');
  }
});

// POST route to add a new item to the pantry
router.post('/add', async (req, res) => {
  ...
  try {
    const queryText = `
      INSERT INTO pantry (user_id, item_name, quantity, date_bought, expiration_date, calories, fat, protein, carbs)
    ...
    res.status(500).send('Server error adding pantry item');
  }
});

// DELETE route to remove pantry item by ID
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const queryText = 'UPDATE pantry SET quantity = $1 WHERE id = $2';
